...
CLOUD_PROVIDERS = {
    "anthropic": {
        ...
        # claude-opus-4-5 and any fable model are deliberately NOT tiered here.
        # Rule 8 (CLAUDE.md): they must never be auto-picked by build_routing —
        # only sonnet is used for the "high" auto-tier; Opus/Fable require an
        # explicit user ask every time, regardless of complexity score.
        "models":   ["claude-haiku-4-5", "claude-sonnet-4-5"],
        # No model is tagged "high" — complex_pick falls back to medium_pick
        # (sonnet) via build_routing's fallback chain instead of reaching opus.
...
def write_model_env(providers: list[dict], routing: dict):
    model_env = find_project_root() / ".model.env"
    lines = [
...
def format_context(project: dict, providers: list[dict], routing: dict, model_env_written: bool, domain_skill: tuple = (None, None)) -> str:
    ...
    lines.append("  │     → Use PROGRESSIVE DISCLOSURE — do NOT dump all 62 skills.")
    ...
    lines.append("  │       • Deep bench: 62 domain specialists load only when your work matches")
