...
- Copies all 63 skills + 10 agents into `~/.claude/` — available in every Claude Code session from now on
...
- Copies all 63 skills + 10 agents into `$env:USERPROFILE\.claude\`
- Registers the MCP server globally
...
| All 63 skills + 10 agents | Each user's `~/.claude/` | Provisioned immediately |
...
| All 63 skills + 10 agents | Each user's `%USERPROFILE%\.claude\` | Provisioned immediately |
...
This wires `%USERPROFILE%\.claude\` with all 63 skills, 10 agents, and the global CLAUDE.md for the new user. They're ready to open Claude Code immediately.
...
3. That's it — all 63 skills load automatically
...
─────────────────────────────────────────────────
  ...
  Guards active. 63 skills loaded.
