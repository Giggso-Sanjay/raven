...
def _mod():
    spec = importlib.util.spec_from_file_location("ss", _ROOT / "scripts" / "session-start.py")
...
def test_no_providers_yields_no_invented_model():
    """The regression itself: empty in must not become a model out."""
...
def test_unconfigured_message_says_the_router_is_advisory():
    """Accurate AND non-alarming — four bare FAILs would misread as breakage."""
    m = _mod()
    out = "\n".join(m.validate_routing(m.build_routing([])))
...
def test_a_genuinely_missing_tier_still_fails():
    """Partial config must not be smoothed over by the UNCONFIGURED shortcut."""
    m = _mod()
...
def test_rule_8_guard_still_fires():
    """Opus/Fable must never pass the self-check, unconfigured shortcut or not."""
    m = _mod()
...
def test_build_routing_never_returns_opus_or_fable():
    """Rule 8 at the source: no provider table may auto-tier either."""
    m = _mod()
