...
- First load / session start: also run `python3 scripts/routing/model-router.py --session-start` and print that banner (version, host, expected SIMPLE/MEDIUM/COMPLEX).
...
5. If `educate=guided` (default): briefing (WHAT/HOW/files, ≤200 words) then **STOP**. No Write/Edit/`spawn_subagent` that mutates until the user says **go ahead** / **approved** / **proceed**. `educate off` / Lucky persist in `.raven/educate.json`. Grok/Codex/Cursor/Windsurf/VS Code have no PreToolUse — skipping this loop is the same failure as empty dashboard Logs.
